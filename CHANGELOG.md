...
- **web 业务错误码透传**：`createHandler` 此前将 controller 返回的任何 error 一律映射为 `500 + 50000`，导致 `api/core` 错误码体系在业务路径形同虚设。现在：
  - controller 返回 `*core.Error`（含 `%w` 包装链，`errors.As` 识别）→ 响应透传其 `code` + `message`
  - HTTP 状态码按错误码映射：40100→401、40300→403、40400→404、40500→405、40900→409、42200→422、50100→501、50200→502、50300→503、50400→504；其余 4xxxx→400、5xxxx→500、自定义 60000-99999→400
  - 普通 error → 仍为 `500 + 50000`（不透传内部细节）；panic → 500 不变；`Details`/`Cause` 仅进日志不进响应体（防泄露）
  - 新增单测：透传 / 包装链 / 普通 error / panic / 成功响应 / 参数错误全覆盖
...
- **examples/demo**：完整可运行示例（分层架构 / DB / 缓存 / JWT / 方法级中间件，SQLite + 本地缓存零外部依赖）

### 修复

- **`landc init` 选项失效**：`--path/--module/--force/--no-git/--no-readme` 从未注册到命令框架，`--path` 实际被当作无值选项、路径变成位置参数（已在当前目录误建项目）；`gen` 子命令 `--module`、`doctor --check-network` 同类问题一并修复
- **frame 依赖版本落后**：`frame/go.mod` require `tools v0.5.0`，但 frame 已使用 v0.7.0 新增的 `tools/security`、`tools/ratelimit`、`tools/verifycode` 包（go.work 本地模块掩盖），发布后下游将编译失败——已升级为 `tools v0.7.0` 并独立构建验证

### 文档

- README 重写为 8 步上手指南（路由 / 参数绑定 / 统一响应 / 配置 / 数据库 / 缓存 / JWT / 工程化），代码经编译验证
