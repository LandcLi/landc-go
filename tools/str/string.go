...
func SubStringBetween(s string, start string, end string) string {
...
func SubStringBefore(s string, sep string) string {
...
// RemoveAll 移除所有指定子串
func RemoveAll(s string, substr string) string {
...
func ToPinyinWithSep(s string, sep string) string {
	args := pinyin.NewArgs()
	args.Style = pinyin.Normal
	args.Heteronym = false
	pinyinSlice := pinyin.Pinyin(s, args)
